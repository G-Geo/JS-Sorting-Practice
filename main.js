...
const stringsToNumbs = (numbers) => {
  piAsNumbers = numbers.map(res => res * 1)
  console.log(piAsNumbers)
  return piAsNumbers
}
...
const sumOfEvens = (numbers) => {
    sumEvents = numbers.forEach(number =>{
        if (number % 2 == 0){
...
const thisQuestionIsConfusing = (numbers) => {
   
   for (let i = 0; i < numbers.length; i++) {
      let placeHolder = 0
      placeHolder = numbers[i] + i
      if (placeHolder == 512){
         atxIdx = i
         return
      }
...
thisQuestionIsConfusing(piAsNumbers)
console.log(`index: ${atxIdx}, value: ${piAsNumbers[atxIdx]}`);
...
const uniqueArrayCreator = (oArray) =>{
    let nArray = [];
    for (let i of oArray){
        nArray[i.weather_state_name] = true
    }
    nArray = Object.keys(nArray)
    console.log(nArray)
}

uniqueArrayCreator(weather)

//find the id of the object in weather that has a min_temp of 15.915

let idealTemp = ""

weather.find(function(element, index){
  if (element.min_temp === 15.915){
    idealTemp = element.id
  }
})
